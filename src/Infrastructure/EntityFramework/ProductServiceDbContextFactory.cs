...
namespace SoftSentre.Shoppingendly.Services.Products.Infrastructure.EntityFramework
{
    public class ProductServiceDbContextFactory : IDesignTimeDbContextFactory<ProductServiceDbContext>
    {
        public ProductServiceDbContext CreateDbContext(string[] args)
        {
            ...
            var loggerFactory =
                new SerilogLoggerFactory(new SerilogConfigurator(applicationService).CreateSerilogLogger(environment));

            return new ProductServiceDbContext(loggerFactory, sqlSettings, optionsBuilder.Options);
        }
    }
}